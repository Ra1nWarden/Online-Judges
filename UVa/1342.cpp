...
int main() {
  ...
  while(~scanf("%d", &n) && n) {
	...
	Line l2(points[j], points[(j+1) % n]);
	if(SegIntersectSeg(l1, l2)) {
	  pair<int, Point> intersection = l1 & l2;
	  vertices[vi++] = intersection.second;
	}
      }
    }
    sort(vertices, vertices + vi);
    vi = unique(vertices, vertices + vi) - vertices;
    for(int i = 0; i < vi; i++) {
      for(int j = 0; j < n; j++) {
	if(vertices[i] == points[j] || vertices[i] == points[(j+1) % n])
	  continue;
	Line l(points[j], points[(j + 1) % n]);
	if(PointOnSeg(vertices[i], l)) {
	  e++;
	}
      }
    }
