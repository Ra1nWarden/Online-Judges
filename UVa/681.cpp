...
int main() {
  ...
  for(int i = 0; i < tc; i++) {
    ...
    printf("%d\n", top+1);
    int startj = 0;
    Point startp = list[Stack[0]];
    for(int j = 1; j < top; j++) {
      if(list[Stack[j]].y < startp.y) {
	startp = list[Stack[j]];
	startj = j;
      } else if (list[Stack[j]].y == startp.y && list[Stack[j]].x < startp.x) {
	startp = list[Stack[j]];
	startj = j;
      }
    }
    for(int j = startj; j <= startj + top; j++) {
      printf("%.0f %.0f\n", list[Stack[j % top]].x, list[Stack[j % top]].y);
    }
  }
  return 0;
}
