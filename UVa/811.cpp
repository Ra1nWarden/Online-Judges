...
int main() {
  ...
  while(scanf("%d", &n) != EOF && n) {
    for(int i = 0; i < n; i++) {
      scanf("%lf %lf %d %d", &points[i].x, &points[i].y, &val[i], &length[i]);
    }
    int bestv = (1 << MAXN) - 1;
    double bestcost = INF;
    double woodleft = 0.0;
	...
	if(bitAt(bitmask, j)) {
	  ...
	  totallength += (double) length[j];
	  totalcost += (double) val[j];
	}
      }
      if(sgn(totalcost - bestcost) > 0)
	continue;
      Graham(l);
      double peri = 0.0;
      for(int j = 0; j < top; j++) {
	peri += dist(list[Stack[j]], list[Stack[(j+1) % top]]);
      }
      if(sgn(peri - totallength) > 0)
	continue;
      if(sgn(totalcost - bestcost) == 0) {
	int bestcount = 0;
	int curcount = 0;
